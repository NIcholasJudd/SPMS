...
myApp.controller("EffortCtrl", ['$scope', 'ProjectFactory',
    function ($scope, $location) {
        ...
        $scope.submitFunctionPoints = function () {
            ...
            for (var i = 0; i < 5; i++) {
                $scope.functionPointCount.value += (Number($scope.functionPoints[i].count1) * Number($scope.functionPoints[i].low));
                $scope.functionPointCount.value += (Number($scope.functionPoints[i].count2) * Number($scope.functionPoints[i].med));
                $scope.functionPointCount.value += (Number($scope.functionPoints[i].count3) * Number($scope.functionPoints[i].high));
            }
