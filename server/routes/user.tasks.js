...
var userTask = {
    ...
    getAll: function(req, res) {
        console.log('usertaskemail: ', req.params.email);
        pg.connect(connectionString, function(err, client, done) {
            if(err) {
                console.error(err.stack);
                return res.status(500).send(err);
            }
            var query = client.query("select * from task where task_id IN (select task_id from task_role where email = $1);", [req.params.email], function(err, result) {
                if(err) {
                    console.error(err.stack);
                    return res.status(500).send(err);
                }
